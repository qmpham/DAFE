...
class Multi_domain_SelfAttentionDecoder_WDC(Decoder):
  ...
  def _run(self,
    ...
    # Run each layer.
    new_cache = []
    for i, layer, domain_share_gate, domain_specific_gate, feed_forward in enumerate(self.layers, self.domain_share_gates, self.domain_specific_gates, self.feed_forwards):
      inputs, layer_cache, attention = layer(
          inputs,
          mask=mask,
          memory=memory,
          ...
          cache=cache[i] if cache is not None else None,
          training=training)
      c_r = domain_share_gate(inputs, memory = h_r, mask = encoder_mask)
      c_s = domain_specific_gate(inputs, memory = h_s, mask = encoder_mask)
