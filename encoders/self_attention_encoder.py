...
class Multi_domain_SelfAttentionEncoder_v2(Encoder):
  ...
  def call(self, inputs, sequence_length=None, training=None):
    ...
    for layer, multi_domain_layer in zip(self.layers,self.multi_domain_layers):
      inputs = layer(inputs, mask=mask, training=training)
      if self.ADAP_layer_stopping_gradient:
        inputs = multi_domain_layer(tf.stop_gradient(inputs), domain, mask=tf.cast(mask,tf.float32), training=training) + inputs
      else:
        inputs = multi_domain_layer(inputs, domain, mask=tf.cast(mask,tf.float32), training=training) + inputs
    outputs = self.layer_norm(inputs)
  ...
  def forward_fn(self, inputs, args_dict, sequence_length=None, training=None):
    ...
    #for layer in self.layers:
    for layer, multi_domain_layer in zip(self.layers,self.multi_domain_layers):
      inputs = layer.forward_fn(inputs, args_dict, mask=tf.cast(mask,tf.float32), training=training)
      if self.ADAP_layer_stopping_gradient:
        inputs = multi_domain_layer.forward_fn(tf.stop_gradient(inputs), args_dict, domain, mask=tf.cast(mask,tf.float32), training=training) + inputs
      else:
        inputs = multi_domain_layer.forward_fn(inputs, args_dict, domain, mask=tf.cast(mask,tf.float32), training=training) + inputs
    outputs = self.layer_norm.forward_fn(inputs, args_dict)
    return outputs, None, sequence_length
