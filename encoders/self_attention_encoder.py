...
class Multi_domain_SelfAttentionEncoder_v16(Encoder):
  ...
  def __init__(self,
    ...
    self.noisy_layers = [multi_domain_adapter_class(num_units, num_domain_units, num_units, domain_numb=2, inner_layer_norm=inner_layer_norm, name="noisy_ADAP_%d"%i) for i in range(num_layers)]
    self.multi_domain_gate = multi_domain_adapter_gate_class(num_units, num_units, domain_numb=num_domains, name="ADAP_gate")
