...
from layers.layers import Multi_domain_FeedForwardNetwork, Multi_domain_FeedForwardNetwork_v2, Multi_domain_FeedForwardNetwork_v3, DAFE, Multi_domain_Gate, Multi_domain_Gate_v2
...
class Multi_domain_SelfAttentionEncoder_v6(Encoder):
  ...
  def __init__(self,
    ...
    self.layers = [
        ...
        for i in range(num_layers)]    
    self.multi_domain_layers = [
        multi_domain_adapter_class(num_units, num_domain_units, num_units, domain_numb=num_domains, name="ADAP_learning_%d"%i)
        for i in range(num_layers)]
