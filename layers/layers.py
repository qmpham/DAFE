...
class Multi_domain_FeedForwardNetwork_v9(tf.keras.layers.Layer):
  ...
  def call(self, inputs, domain, mask=None, training=None):  # pylint: disable=arguments-differ
    ...
    outputs = tf.map_fn(lambda x: tf.transpose(tf.nn.bias_add(tf.matmul(tf.transpose(x[0]), x[1] , transpose_b=self.outer_transpose), x[2])), (inner, self.outer_kernel, self.outer_bias), dtype=tf.float32, parallel_iterations=self.domain_numb)
    ...
    outputs = tf.map_fn(lambda x: tf.reduce_sum(tf.reshape(x[0] * tf.tile(tf.reshape(tf.transpose(tf.tile(tf.expand_dims(x[1],0),[self.output_dim,1])),[1,-1]),[tf.shape(x[0])[0],1]), [-1, self.domain_numb, self.output_dim]),1), (outputs, domain), dtype=tf.float32, parallel_iterations=100)
