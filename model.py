...
class LDR_SequenceToSequence(model.SequenceGenerator):
  ...
  def __init__(self,
    ...
    self.decoder = decoder
    self.share_embeddings = share_embeddings
    self.ldr_emb = self.add_weight(
        "ldr_embedding",
        [vocab_size, num_domain_units * num_domains],
        initializer=None,
        trainable=self.trainable)
    self.domain_mask = make_domain_mask(num_domains, num_domain_units=num_domain_units)
    with tf.name_scope("ldr"):
  ...
  def call(self, features, labels=None, training=None, step=None):
    ...
    source_length = self.features_inputter.get_length(features)
    source_inputs = self.features_inputter(features, training=training)
    ldr_inputs = tf.nn.embedding_lookup(self.ldr_emb, features["ids"])
    ...
    source_inputs = self.fusion_layer(tf.concat([source_inputs, ldr_inputs],2))
    encoder_outputs, encoder_state, encoder_sequence_length = self.encoder(
        [source_inputs, features["domain"]], sequence_length=source_length, training=training)
