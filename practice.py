  ...
  if args.run == "metatrainv7":
    ...
    for i in range(num_domains-1):
      for j in range(i+1,num_domains):
        ...
        model.params.update({"average_loss_in_time": True})
        model.params.update({"beam_width": 5})
        proxy_distance = task.train_tag(local_config, local_optimizer, local_learning_rate, local_model, strategy, local_checkpoint_manager, local_checkpoint, experiment="baseline", save_every=local_config.get("save_every",1000), eval_every=local_config.get("eval_every",2000))
