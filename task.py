...
def train_NGD(config,
  ...
  train_dataset = create_trainining_dataset(strategy, model, domain, source_file, target_file, batch_train_size, batch_type, shuffle_buffer_size, 
                                            ...
                                            multi_domain=config.get("multi_domain", True), picking_prob=config.get("picking_prob",None), 
                                            temperature=config.get("temperature",1.0))
  hessian_datasets = create_async_trainining_dataset(strategy, model, domain, source_file, target_file, batch_hessian_size, "examples", shuffle_buffer_size, 
                                            maximum_length, length_bucket_width=config.get("length_bucket_width",1), 
  ...
  def normalize_hessian_accumulators():
    sum = 0
    for h in hessian_accumulators.hessians:
  ...
  def _accumulate_diag_hessians(source,target): 
    ...
    with tf.GradientTape(persistent=True) as tape:  
      ...
      def hessian_accum_along_loss(diag_hessian_acc, x):
        ...
        hessian_accumulators(gradients)
        ...
        for grad, acc in zip(gradients, hessian_accumulators):
          if isinstance(grad, tf.IndexedSlices):
            ...
            acc.assign(tf.tensor_scatter_nd_add(acc.value(), tf.expand_dims(grad.indices,1), grad.values * grad.values))
            continue
          else:
  ...
  def _accumulate_gradients(source, target):
    ...
    for gradient, hessian_moving_stat in zip(gradients, hessian_moving_stats):
      if isinstance(gradient,tf.IndexedSlices):
        new_gradients.append(tf.IndexedSlices(gradient.values / tf.nn.embedding_lookup(hessian_moving_stat.value(), gradient.indices), 
        gradient.indices, dense_shape=gradient.dense_shape))
      else:
        new_gradients.append(gradient / hessian_moving_stat)
