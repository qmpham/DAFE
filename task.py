  ...
  with _summary_writer.as_default():
    while True:
      ...
      if step % gumbel_temperature_decay==0:
        gumbel_temperature.assign(tf.cast(tf.math.maximum(0.2, tf.math.exp(-r*step)),tf.float32))
      if step % save_every == 0:
        tf.get_logger().info("Saving checkpoint for step %d", step)
        checkpoint_manager.save(checkpoint_number=step)
      if step % eval_every == 0:
...
def translate_sparse_layer(source_file,
  ...
  domain_dropout_mask = tf.cast(tf.reshape(tf.transpose(tf.tile(tf.expand_dims(tf.math.argmax(unit_selection_logits,1),0),[model.unit_group_size,1])),[-1]),tf.float32)
  tf.print("dropout_mask:",domain_dropout_mask,summarize=-1)
  tf.print("dropout_logits",unit_selection_logits,summarize=-1)
  tf.print(tf.math.argmax(unit_selection_logits,1),summarize=-1)
  @tf.function
  def predict_next():    
    source = next(iterator)
    source_length = source["length"]
