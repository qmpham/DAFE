  ...
  def _accumulate_NGD_gradients(source, target):
    ...
    for gradient, hessian_moving_stat, var in zip(gradients, hessian_moving_stats, variables):
      if isinstance(gradient,tf.IndexedSlices):
        ...
        new_gradients.append(gradient / (hessian_moving_stat.value() +epsilon) * 1 / tf.sqrt(tf.reduce_sum(tf.square(gradient) / tf.square(hessian_moving_stat.value()+epsilon) * hessian_moving_stat.value())))
