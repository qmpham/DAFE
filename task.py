...
import numpy as np
from utils.dataprocess import create_trainining_dataset_with_dprob, create_trainining_dataset_hvd, merge_map_fn, create_trainining_dataset_v1, create_multi_domain_meta_trainining_dataset_v2, create_meta_trainining_dataset, create_trainining_dataset, create_multi_domain_meta_trainining_dataset, create_trainining_dataset_v2, create_multi_domain_meta_trainining_dataset_v1
...
def train(config,
  ...
  @dataset_util.function_on_next(train_dataset)
  def _train_adv_forward(next_fn):    
    with strategy.scope():
      per_replica_source, per_replica_target = next_fn()
      per_replica_loss, per_replica_num_examples = strategy.experimental_run_v2(
  ...
  start = time.time()  
  train_data_flow = iter(_train_forward())
  train_adv_data_flow = iter(_train_adv_forward())
  _, _ = next(train_data_flow)
      ...
      for _ in range(int(config.get("accumulation_step",1))):
        if config.get("adv_step",None):
          if step < config.get("adv_step",None):
            ...
            loss, num_examples = next(train_adv_data_flow)    
            _loss.append(loss)
            _number_examples.append(num_examples)
        else:
          loss, num_examples = next(train_data_flow)    
...
def train_adv(config,
  ...
  adv_train_dataset = create_trainining_dataset(strategy, model, domain, source_file, target_file, batch_train_size, batch_type, shuffle_buffer_size, 
                                            maximum_length, length_bucket_width=config.get("length_bucket_width",1), 
                                            multi_domain=config.get("multi_domain", True),picking_prob=config.get("picking_prob",None), temperature=config.get("temperature",1.0))
  ...
  def _accumulate_adv_gradients(source, target):
    true_domain = source["domain"][0]
    #tf.print("true domain: ", true_domain)
  ...
  start = time.time()  
  train_data_flow = iter(_train_forward())
  train_adv_flow = iter(_train_adv_forward())
  _, _ = next(train_data_flow)
