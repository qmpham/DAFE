...
import numpy as np
from utils.dataprocess import merge_map_fn, create_trainining_dataset_v1, create_meta_trainining_dataset, create_trainining_dataset, create_multi_domain_meta_trainining_dataset, create_trainining_dataset_v2, create_multi_domain_meta_trainining_dataset_v1
...
def translate(source_file,
  ...
  def predict_next():    
    ...
    if experiment in ["residual","residualv2","residualv1","residualv3","residualv5","baselinev1"]:
...
def train_v8(config,
  ...
  def _accumulate_gradients(meta_train_source, meta_train_target, meta_test_source, meta_test_target):     
    ...
    for v in variables:
      args_dict.update({v.name:v})
    gradients = optimizer.gradient(training_loss, variables)  
    gradient_accumulator(gradients)
    ...
    loss = model.compute_loss(outputs, meta_test_target, training=True)
    meta_training_loss = loss[0] / loss[1]
    gradients = optimizer.gradient(meta_training_loss, variables)
    gradient_accumulator(gradients)
    num_word_examples = tf.reduce_sum(meta_test_target["length"]) + tf.reduce_sum(meta_train_target["length"])
