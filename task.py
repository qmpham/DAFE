...
def train_tag(config,
  ...
  with _summary_writer.as_default():
    while True:
      ...
      if step > train_steps:        
        checkpoint_path = checkpoint_manager.latest_checkpoint
        for src,ref,i in zip(config["test_src"],config["test_ref"],config["test_domain"]):
          output_file = os.path.join(config["model_dir"],"eval",os.path.basename(src) + ".trans." + os.path.basename(checkpoint_path))
          score = translate(src, ref, model, checkpoint_manager, checkpoint, i, output_file, length_penalty=config.get("length_penalty",0.6), experiment=experiment)
          tf.summary.scalar("test_score_%d"%i, score, description="BLEU on test set %s"%src)
          errors.append(1-score/100)
        break
  return (2*(1-np.mean(errors)))
