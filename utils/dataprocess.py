...
def create_multi_domain_meta_trainining_dataset(strategy, model, domain, source_file, target_file, batch_meta_train_size, batch_meta_test_size, batch_type, shuffle_buffer_size, maximum_length):
  ...
  meta_train_dataset = meta_train_dataset.map(make_batch_per_replica_1_(strategy.num_replicas_in_sync*2))
  meta_test_dataset = meta_test_dataset.map(make_batch_per_replica_1_(strategy.num_replicas_in_sync*2))
  with strategy.scope():
